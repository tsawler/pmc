...
if (request.getParameter("d-443587-p") != null) {
	tablepager = request.getParameter("d-443587-p");
	session.setAttribute("d-443587-p", tablepager + "");
}

if (request.getParameter("pagesize") != null) {
	pagesize = Integer.parseInt((String) request.getParameter("pagesize"));
	session.setAttribute("faqPageSize", pagesize + "");
} else if (session.getAttribute("faqPageSize") != null) {
	pagesize = Integer.parseInt((String) session.getAttribute("faqPageSize"));
}

faq.setConn(conn);
if (request.getParameter("searchName") != null) {
	faq.createCustomWhere(" and question ilike '" + request.getParameter("searchName") + "%' ");
	faqName = request.getParameter("searchName");
}

RowSetDynaClass resultset = faq.getAllFaqsDynaBean();
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->Maintain FAQ Entries<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div align="center">
		<fieldset><legend>Maintain FAQ Entries</legend>
		<form method="post" action="/faq/jpage/1/p/FaqChoose/admin/1/content.do">
		<table class="tabledata">
		  <tr>
		    <td>Question (all or partial)</td>
			<td>Paging</td>
		  </tr>
		  <tr>
		    <td>
		  ...
		  <ajax:autocomplete
			  ...
			  parameters="searchname={searchName},table=faq,field=question"
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/faq/jpage/1/p/FaqChoose/admin/1/content.do" export="false" defaultsort="1" id="faqObj" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="question" title="Question" href="/admin/jpage/1/p/EditFaqEntry/a/editfaqentry/class/faq/admin/1/content.do?edit=false&cancelPage=/faq/jpage/1/p/FaqChoose/admin/1/content.do" paramId="id" paramProperty="faq_id" sortable="true" headerClass="sortable" />
