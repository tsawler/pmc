...
<jsp:useBean id="news" scope="request" class="com.verilion.database.NewsItem" />
<%
String lastName = "";
String query = "";
int pagesize = 10;
...
if (request.getParameter("searchName") != null) {
	news.setSCustomWhere(" where news_title ilike '" + request.getParameter("searchName") + "%' ");
	lastName = request.getParameter("searchName");
} else {
	news.setSCustomWhere("");
}

RowSetDynaClass resultset = news.getAllNewsItemsDynaBean();
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->News Items<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div align="center">
		<fieldset><legend>Maintain news items</legend>
		<form method="post" action="/news/jpage/1/p/NewsItemChoice/admin/1/content.do">
		<table class="tabledata">
		  <tr>
		    <td>News item name  (all or partial)</td>
			<td>Paging</td>
		  </tr>
		  <tr>
		    <td>
		  ...
		  <ajax:autocomplete
			  ...
			  parameters="searchname={searchName},table=news,field=news_title"
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/news/jpage/1/p/NewsItemChoice/admin/1/content.do" export="false" defaultsort="1" id="currentRowObject" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="news_title" title="News Item" href="/admin/jpage/1/p/NewsItemDetail/a/newsitemdetail/class/newsobject/admin/1/content.do?cancelPage=/news/jpage/1/p/NewsItemChoice/admin/1/content.do&edit=false" paramId="id" paramProperty="news_id" sortable="true" headerClass="sortable" />
			  <display:column property="news_start_date" title="Start date" sortable="true" headerClass="sortable" />
			  <display:column property="news_active_status" title="Status" />
