...
GenericTable myTableObject = new GenericTable(" pmc_member_feedback mf left join customer c on  (mf.customer_id = c.customer_id) ");
myTableObject.setConn(conn);
myTableObject.setSSelectWhat(" mf.*, c.username ");
myTableObject.setSCustomOrder(" order by date_time desc ");
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/feedbackchoose/jpage/1/p/Feedback/admin/1/content.do" export="true" id="mflinksobj" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="username" title="Username" />
			 <display:column property="rate_site" title="Site Rating" />
			<display:column property="like_most" title="Likes" />
			<display:column property="like_least" title="Dislikes" />
			<display:column property="improve" title="How to improve" />
			<display:column property="date_time" title="Date/Time" />
