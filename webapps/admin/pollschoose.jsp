...
GenericTable myTableObject = new GenericTable(" polls ");
myTableObject.setConn(conn);
myTableObject.setSSelectWhat(" poll_id, title, number_voters, case when active_yn = 'y' then '<span style=\"color: green\">active</span>' else '<span style=\"color: red\">inactive</span>' end as status ");
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->Maintain Polls<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/pollschoose/jpage/1/p/PollsChoose/admin/1/content.do" export="false" id="plinksobj" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="title" title="Poll" href="/admin/jpage/1/p/EditPoll/a/editpoll/class/polls/admin/1/content.do?id=0&edit=false&cancelPage=/pollschoose/jpage/1/p/PollsChoose/admin/1/content.do?edit=false" paramId="id" paramProperty="poll_id"  />
			 <display:column property="number_voters" title="Votes" />
