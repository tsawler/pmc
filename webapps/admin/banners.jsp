...
<jsp:useBean id="banners" scope="request" class="com.verilion.database.Banners" />
<%
String lastName = "";
String query = "";
int pagesize = 10;
...
banners.setConn(conn);
if (request.getParameter("searchName") != null) {
	banners.setSCustomWhere(" and banner_name ilike '" + request.getParameter("searchName") + "%' ");
	lastName = request.getParameter("searchName");
} else {
	//news.setSCustomWhere("");
}

RowSetDynaClass resultset = banners.getAllBannersDynaBean();
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->Maintain Banners<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div align="center">
		<fieldset><legend>Maintain banners</legend>
		<form method="post" action="/banners/jpage/1/p/MaintainBanners/admin/1/content.do">
		<table class="tabledata">
		  <tr>
		    <td>Banner Name  (all or partial)</td>
			<td>Paging</td>
		  </tr>
		  <tr>
		    <td>
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/banners/jpage/1/p/MaintainBanners/admin/1/content.do" export="false" defaultsort="1" id="currentRowObject" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="banner_name" title="Banner Name" href="/admin/jpage/1/p/EditBanner/a/editbanner/admin/1/content.do?edit=false&cancelPage=/banners/jpage/1/p/MaintainBanners/admin/1/content.do" paramId="id" paramProperty="banner_id" sortable="true" headerClass="sortable" />
