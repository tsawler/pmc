...
if (request.getParameter("d-3586625-p") != null) {
	tablepager = request.getParameter("d-3586625-p");
	session.setAttribute("d-3586625-p", tablepager + "");
}

if (request.getParameter("pagesize") != null) {
	pagesize = Integer.parseInt((String) request.getParameter("pagesize"));
	session.setAttribute("faqCatPageSize", pagesize + "");
} else if (session.getAttribute("faqCatPageSize") != null) {
	pagesize = Integer.parseInt((String) session.getAttribute("faqCatPageSize"));
}

faq.setConn(conn);
if (request.getParameter("searchName") != null) {
	faq.createCustomWhere(" and faq_categories_name ilike '" + request.getParameter("searchName") + "%' ");
	faqName = request.getParameter("searchName");
}

RowSetDynaClass resultset = faq.getAllFaqCategoriesDynaBean();
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->Maintain FAQ Categories<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div align="center">
		<fieldset><legend>Maintain FAQ Categories</legend>
		<form method="post" action="/faqcategories/jpage/1/p/FaqChooseCategory/admin/1/content.do">
		<table class="tabledata">
		  <tr>
		    <td>Category (all or partial)</td>
			<td>Paging</td>
		  </tr>
		  <tr>
		    <td>
			<input type="text" name="searchName" id="searchName" class="form-autocomplete" size="40" value="<%=faqName%>" />
			</td>
			<td>
		  ...
		  <ajax:autocomplete
			  ...
			  parameters="searchname={searchName},table=faq_categories,field=faq_category_name"
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/faqcategories/jpage/1/p/FaqChooseCategory/admin/1/content.do" export="false" id="faqCatObj" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="faq_categories_name" title="Category" href="/admin/jpage/1/p/EditFaqCategory/a/editfaqcategory/class/faqcat/admin/1/content.do?edit=false&cancelPage=/faqcategories/jpage/1/p/FaqChooseCategory/admin/1/content.do" paramId="id" paramProperty="faq_categories_id"  />
