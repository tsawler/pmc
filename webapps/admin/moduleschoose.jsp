...
GenericTable myTableObject = new GenericTable(" modules ");
myTableObject.setConn(conn);
myTableObject.setSSelectWhat(" module_id, module_name, case when module_active_yn = 'y' then '<span style=\"color: green\">active</span>' else '<span style=\"color: red\">inactive</span>' end as status ");
myTableObject.setSCustomOrder(" order by module_name ");
myTableObject.setSCustomWhere("");

RowSetDynaClass resultset = myTableObject.getAllRecordsDynaBean();
...
<table style="padding: 0; border: 1px solid silver; height: 95%; width: 100%;">
  ...
  <tr>
    ...
    <td class="tabledata" style="border: 1px solid silver;" valign="top" width="83%" cellpadding="0" cellspacing="0">
        ...
        <tr style="height: 15px;">
        <td class="tableheader"><!-- InstanceBeginEditable name="Title" -->Maintain Modules<!-- InstanceEndEditable --></td>
        </tr>
		...
		<div>
			<display:table name="requestScope.results.rows" requestURI="/moduleschoose/jpage/1/p/ModulesChoose/admin/1/content.do" export="false" id="modssobj" pagesize="<%=pagesize%>"  class="tabledatacollapse" style="width: 100%" >
			 <display:column property="module_name" title="Module" href="/admin/jpage/1/p/ModuleAdd/a/editmodule/admin/1/content.do?cancelPage=/moduleschoose/jpage/1/p/ModulesChoose/admin/1/content.do&edit=false&cancelPage=/quoteschoose/jpage/1/p/QuotesChoose/admin/1/content.do" paramId="id" paramProperty="module_id"  />
