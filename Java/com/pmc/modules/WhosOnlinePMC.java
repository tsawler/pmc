	...
	public String getHtmlOutput(Connection conn, HttpSession session,
		...
		try {
			theString = (new StringBuilder(
					"<div class=\"vmodule\">\n<div class=\"vmoduleheading\">Who's Online?</div>\n<div class=\"vmodulebody\">We have "))
					.append(sessionCount).append(visitorString)
					.append("online.</div>").toString();
			...
			if (drs.next()) {
				drs.previous();
				theString = (new StringBuilder(String.valueOf(theString)))
						.append("<div class=\"vmodulebody\"><br /><strong>Members online:</strong><br />")
						.toString();
				while (drs.next())
					if (session.getAttribute("customer_id") != null) {
						if (Integer.parseInt((String) session
								.getAttribute("customer_id")) > 0)
							theString = (new StringBuilder(
									String.valueOf(theString)))
									.append("&nbsp;&nbsp;<a title=\"Read  member profile\" href=\"/members/jpage/1/p/MemberDirectory/content.do?uname=")
									.append(drs.getString("username"))
									.append("\">")
									.append(drs.getString("username"))
			...
			Errors.addError((new StringBuilder(
					"SessionCounter.getHtmlOutput:Exception:")).append(
					e.toString()).toString());
		}
		return theString;
	}
