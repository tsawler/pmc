...
public class ProcessJoinPMC extends ProcessPage {
   ...
   public HTMLTemplateDb BuildPage(
      ...
      try {
         ...
         MasterTemplate.searchReplace("$DDLBIY$", Years.makeDropDownListYears(
               "implant_year",
               0,
               1950,
               year));

         MasterTemplate.searchReplace("$DDLBBY$", Years.makeDropDownListYears(
               "birth_year",
               0,
               1870,
               year));
   ...
   public HTMLTemplateDb ProcessForm(
      ...
      try {
         ...
         device_type = Integer.parseInt((String) request
         ...
         number_of_devices = Integer.parseInt((String) request
               .getParameter("number_of_devices"));
         interests = (String) request.getParameter("interests");
         implant_year = Integer.parseInt((String) request
               .getParameter("implant_year"));
         birth_year = Integer.parseInt((String) request
         ...
         customer_add_to_mailing_list = Integer.parseInt((String) request
         ...
         rs = st
               .executeQuery("select count(customer_id) from v_pmc_customer where customer_email = '"
                     + email_address
                     + "'");
         while (rs.next()) {
            numRecs = rs.getInt(1);
         ...
         st = conn.createStatement();
         rs = st
               .executeQuery("select count(customer_id) from v_pmc_customer where username = '"
                     + username
                     + "'");
         while (rs.next()) {
            numRecs = rs.getInt(1);
         ...
         if (isOkay) {
            ...
            response
                  .sendRedirect("/public/jpage/1/p/Home/a/login/content.do?email_address="
                        + email_address
                        + "&password="
                        + password
            ...
            System.out.println("Error in join: " + theErrorMessage);
            this
                  .BuildPage(
                        request,
                        response,
